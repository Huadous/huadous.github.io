...
## eigvals_cir
```julia
function eigvals_cir(C)
n = size(C,1);
c = C[:,1];
sigma =zeros(n)
zk = x -> exp(-im*2*pi*x*(n-1)/n);
zkn = x -> [x^i for i = 0:n-1]
zk_data = zk.(0:n-1)
for i = 1:n
sigma[i] = norm(sum(c.*zkn(zk_data[i])));
end
return sum(sigma)
end
```
...
function classify_image(test, train, K::Int)
    ...
    Q = mapslices(take,train,dims =(1,2))
    err = zeros(d,p)
    for i = 1:d
        err[i,:] = sum((test-Q[:,:,i]*(Q[:,:,i]'*test)).^2, dims=1)
    end
    return labels = mapslices(x-> findmin(x)[2]-1,err,dims = 1)
end
```